...
export async function createUserToneProfile(userId, input) {
  return await withState((state) => {
    ...
    const targetToneId = String(input?.id || "").trim();
    ...
    const existingCustomToneById = user.toneProfiles.find(
      (profile) => String(profile?.id || "").trim() === targetToneId
    );

    const existingCustomToneByLabel = user.toneProfiles.find(
      (profile) => String(profile?.label || "").toLowerCase() === label.toLowerCase()
    );

    if (targetToneId) {
      if (!existingCustomToneById) {
        throw new Error("Custom tone profile not found.");
      }

      const duplicateLabel = user.toneProfiles.find(
        (profile) =>
          String(profile?.id || "").trim() !== targetToneId &&
          String(profile?.label || "").toLowerCase() === label.toLowerCase()
      );
      if (duplicateLabel) {
        throw new Error("A custom tone with this name already exists.");
      }

      existingCustomToneById.label = label;
      existingCustomToneById.description = description;
      existingCustomToneById.rules = rules;
      existingCustomToneById.isPreset = false;
      user.writingStyle = existingCustomToneById.id;
      user.updatedAt = new Date().toISOString();

      return {
        writingStyle: user.writingStyle,
        writingStyles: listToneProfilesForUser(user),
        createdToneProfile: sanitizeToneProfile(existingCustomToneById),
        mode: "updated"
      };
    }

    if (existingCustomToneByLabel) {
      existingCustomToneByLabel.description = description;
      existingCustomToneByLabel.rules = rules;
      existingCustomToneByLabel.isPreset = false;
      user.writingStyle = existingCustomToneByLabel.id;
      user.updatedAt = new Date().toISOString();

      return {
        writingStyle: user.writingStyle,
        writingStyles: listToneProfilesForUser(user),
        createdToneProfile: sanitizeToneProfile(existingCustomToneByLabel),
        mode: "updated"
      };
    }
...
export async function createDraftFromRun(userId, repoId, runId, options = {}) {
  ...
  const resolvedBrand = resolveBrandProfileForUser(user || null, user?.activeBrandProfile);
  ...
  const generated = await generateReleasePack({
    ...
    brandProfile: resolvedBrand || user?.brandProfile || null,
    aiSettings: sanitizeAiSettings(user?.aiSettings)
  });
