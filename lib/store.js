...
export function writeState(next) {
  ...
  fs.writeFileSync(DB_PATH, JSON.stringify(sanitized, null, 2));
}
...
export function upsertGithubUser(profile, accessToken) {
...
export function getUserWritingPreference(userId) {
  const state = readState();
  const user = state.users.find((item) => item.id === userId);
  return buildUserTonePreferencePayload(user || null);
}

export function updateUserWritingPreference(userId, writingStyleId) {
  return withState((state) => {
    const user = state.users.find((item) => item.id === userId);
    if (!user) {
...
export function createUserToneProfile(userId, input) {
  return withState((state) => {
    const user = state.users.find((item) => item.id === userId);
    if (!user) {
      throw new Error("User not found");
...
export function listConnectedRepos(userId) {
  const state = readState();
...
export function listConnectedReposPage(userId, input = {}) {
...
export function connectSelectedRepos(userId, repos) {
...
export function toggleRepoAutomation(userId, repoId, autoGenerate) {
  return withState((state) => {
...
export function getConnectedRepoById(userId, repoId) {
  const state = readState();
  const repo = state.connectedRepos.find((item) => item.id === repoId && item.userId === userId);
  return repo ? { ...repo } : null;
}

export function recordManualTrigger(userId, repoId, payload) {
  const now = new Date().toISOString();
...
export async function createDraftFromRun(userId, repoId, runId, options = {}) {
  ...
  return withState((state) => {
    ...
    const inboxItem = {
      ...
      userId,
      type: "draft_ready",
      title: `Draft ready: ${run.release.title}`,
...
export function listManualRunsForUser(userId, limit = 20) {
  const state = readState();
...
export function listInboxItemsPage(userId, input = {}) {
...
export function listInboxItems(userId, limit = 30) {
  return listInboxItemsPage(userId, { limit, offset: 0 }).items;
}

export function deleteInboxItem(userId, inboxItemId) {
...
export function listDrafts(userId, limit = 30) {
  const state = readState();
  return state.drafts
...
export function listDraftsPage(userId, input = {}) {
  const rawPage = Number.parseInt(String(input?.page || "1"), 10);
  const rawLimit = Number.parseInt(String(input?.limit || "50"), 10);
  const page = Math.max(1, Number.isFinite(rawPage) ? rawPage : 1);
  const limit = Math.min(100, Math.max(1, Number.isFinite(rawLimit) ? rawLimit : 50));
...
export function updateDraft(userId, draftId, input) {
  return withState((state) => {
