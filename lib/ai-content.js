...
function compactContext(release) {
  ...
  return {
    ...
    commit: context.commit
      ? {
          sha: context.commit.shortSha || context.commit.sha,
          message: context.commit.message,
          author: context.commit.author,
          branch: context.commit.branch,
          additions: context.commit.additions,
          deletions: context.commit.deletions,
          changedFiles: context.commit.changedFiles
        }
      : null,
...
export async function generateReleasePack({
  repoFullName,
  release,
  writingStyleId,
  writingStyle,
  ...
  const textModel = resolveTextModel(aiSettings);
  const imageModel = resolveImageModel(aiSettings);
  ...
  const brandSnapshot = compactBrandProfile(brandProfile);
  ...
  try {
    const prompt = [
      ...
      "- Align copy with brand_context positioning when available.",
      ...
      `brand_context: ${JSON.stringify(brandSnapshot || {})}`
    ].join("\n");
  ...
  try {
    ...
    if (imageModel && isGeminiNanoBananaModel(imageModel.modelId)) {
      const imageTextResult = await withRetry(() =>
        ai.generateText({
...
function compactBrandProfile(profile) {
  ...
  if (
    ...
    Object.keys(normalized.colors).length === 0
  ) {
    return null;
  }
...
function createDefaultImagePrompt({ repoFullName, release, brandProfile }) {
  const palette = [];
  if (brandProfile?.colors?.primary) {
    palette.push(`primary ${brandProfile.colors.primary}`);
  }
  if (brandProfile?.colors?.accent) {
    palette.push(`accent ${brandProfile.colors.accent}`);
  }
  if (brandProfile?.colors?.background) {
    palette.push(`background ${brandProfile.colors.background}`);
  }

  const parts = [
    `Minimal product release card for ${repoFullName}`,
    `tag ${release?.tag || "release"}`,
    `title ${release?.title || "Untitled"}`
  ];

  if (brandProfile?.title) {
    parts.push(`brand ${brandProfile.title}`);
  }
  if (brandProfile?.description) {
    parts.push(`style direction ${brandProfile.description}`);
  }
  if (palette.length > 0) {
    parts.push(`use brand palette: ${palette.join(", ")}`);
  }

  parts.push("clean white background");
  parts.push("cute geeky dev aesthetic");
  parts.push("no text overlap");
