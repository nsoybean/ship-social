...
function normalizeBrandProfiles(rawBrandProfiles) {
  if (!Array.isArray(rawBrandProfiles)) return [];

  return rawBrandProfiles
    .map((item) => {
      if (!item || typeof item !== "object") return null;
...
export async function readState() {
  ...
  const brandProfilesByUserId = new Map();
  for (const row of brandProfilesResult.rows) {
    const userId = String(row.user_id || "");
    if (!userId) continue;

    if (!brandProfilesByUserId.has(userId)) {
      brandProfilesByUserId.set(userId, []);
    }

    brandProfilesByUserId.get(userId).push({
      id: String(row.id || "").trim(),
      label: String(row.label || "").trim(),
  ...
  return sanitizeState({
    users: usersResult.rows.map((row) => ({
      ...
      activeBrandProfile: String(row.active_brand_profile || ""),
      aiSettings: asJson(row.ai_settings, {}),
      brandProfile: asJson(row.brand_profile, {}),
      brandProfiles: brandProfilesByUserId.get(String(row.id || "")) || [],
    ...
    for (const user of state.users) {
      ...
      await client.query(
        ...
        [
          ...
          String(user.activeBrandProfile || ""),
          JSON.stringify(user.aiSettings || {}),
          JSON.stringify(user.brandProfile || {}),
      ...
      const brandProfiles = normalizeBrandProfiles(user.brandProfiles);
      for (const brandProfile of brandProfiles) {
        await client.query(
          ...
          [
            ...
            JSON.stringify(brandProfile.colors || {}),
